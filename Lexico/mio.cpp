...
map< string , map < string, string>	> tabla_sintactica;
		...
		for(int j=0; j<tabla_simbolos.size();j++)
	  		{   
	  			...
	  			if(j==tabla_simbolos.size()-1 and lista_tokens[i].first=="ID")
	  				...
	  				tabla_sintactica_aux.insert(std::pair<string, string>(lista_tokens[i].second.first,a));
	  		
	  		}
	}

	tabla_sintactica_aux.insert(std::pair<string, string>("$",a));
	string NT="";
	tabla_sintactica.insert(std::pair< string, map <string,string>  >(NT,tabla_sintactica_aux));
}
